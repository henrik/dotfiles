...
alias wpus="ruby -e 'loop { x = \`wti push; wti push --target\`; puts x; break unless x.include?(%{Locked}); puts; puts %{#{Time.now.strftime(%{%H:%M:%S})}: Retrying in a bit…}; puts; sleep 15 }'"
