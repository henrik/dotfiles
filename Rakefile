...
module Dotfiles
  ...
  def info(text) = STDOUT.puts("*  #{text}")
  def info_rm(text) = STDOUT.puts("x  #{text}")
...
task :install do
  Dir["*"].each do |file|
    ...
    Dotfiles.symlink(source, File.expand_path("~/.#{basename}"))
  end
  ...
  Dir["config/**/*"].select { |path| File.file?(path) }.each do |path|
    Dotfiles.symlink(File.join(Dir.pwd, path), File.expand_path("~/.#{path}"))
  end
end
